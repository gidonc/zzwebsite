...
this_res_long <- readRDS(paste0(rprojroot::find_rstudio_root_file(), "/playerprofiles/res_long.RDS")) %>%
  filter(Players=="qqplayernameqq", team=="qqcurrentteamqq")
...
qqplayernameqq is currently a player in qqcurrentteamqq.  

They are currently ranked `r this_allmatches_team_rank` in qqcurrentteamqq (`r this_allmatches_overall_rank` in the Zig-Zag family).  

In the last 15 matches they are ranked `r this_recent_team_rank` in qqcurrentteamqq (`r this_recent_overall_rank` in the Zig-Zag family).  

Their best score was qqmaxscoreqq in qqbesteventqq.

Their highest rank within the team was `r bestrank` `r best` 

Their scores relative to the rest of the team are shown in the figure and table below. In the figure, the red line shows the trajectory of qqplayernameqq's scores. The grey line shows the team median and the grey ribbon indicates the range from top five score to bottom five score.

![qqplayernameqq performance relative to each team](qqpathtoimageqq)


### qqplayernameqq's scores
