...
if(readdata==TRUE){
  ...
  gptracker$Players <- unlist(gptracker$Players)
...
all_matches <- all_matches %>%
  ...
  mutate(zzcupst=zz_cups_before/1000,
         ...
         winmargin=abs(zz_points-opp_points),
         magnitude_cupchange=abs(zz_cup_change),
         zz_cup_change = ifelse(is.na(zz_cup_change), zz_cups_after - zz_cups_before, zz_cup_change),
         opp_cup_change = ifelse(is.na(opp_cup_change), opp_cups_after - opp_cups_before, opp_cup_change),
         wincup = ifelse(result=="wins", zz_cups_before, opp_cups_before),
         losecup = ifelse(result=="wins", opp_cups_before, zz_cups_before),
         winrank = ifelse(result=="wins", zz_rank_before, opp_rank_before),
         loserank = ifelse(result=="wins", opp_rank_before, zz_rank_before),
         wincupchange = ifelse(result=="wins", zz_cup_change, opp_cup_change),
         losecupchange = ifelse(result=="wins", opp_cup_change, zz_cup_change),
         cupdiff = wincup-losecup,
         gpdiff = wingp-losegp,
         rankdiff = loserank-winrank,
         cupratio = wincup/losecup,
...
re.playersf1<-ranef(f1)$PlayerTeam %>% 
  ...
  mutate(StdScore = round(lme4::fixef(f1)[1] + StdScore , 0)) %>%
  arrange(-StdScore) %>%
  left_join(results) %>%
