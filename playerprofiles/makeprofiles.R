...
for (n in 1:nrow(players)){
  ...
  res_long %>%
    group_by(event_id, event, round, event_round, team, overall_match_number) %>%
    summarize(med = median(score, na.rm=TRUE), high=quantile(score, probs=.9, na.rm=TRUE), low=quantile(score, probs=.1, na.rm=TRUE)) %>%
    ggplot(aes(overall_match_number, med)) +
    facet_wrap(~team)+
    geom_line(colour="grey70") +
    geom_ribbon(fill="grey50", alpha=.2, aes(ymin=low, ymax=high))+
    theme_bw() +
    geom_line(colour="red", data=thisplayerdat, aes(x=overall_match_number, y=score))
