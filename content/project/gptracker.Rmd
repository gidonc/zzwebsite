---
title: 'Zig-Zag Racerz: Garage Power'
author: "giddybb"
...
the_first <- gplong %>%
  filter(max_gp>3000, n_measures>1, current) %>%
...
## The Middle Order (GP 2000-3000)

```{r, echo=FALSE}
gplong%>%
  filter(max_gp<3000, max_gp>2000, n_measures>1, current) %>%
...
## The Tail Enders (GP under 2000)

```{r, echo=FALSE, fig.height=7}
gplong%>%
  filter(max_gp<2000, n_measures>1, current) %>%
  ggplot(aes(event, GP, group=Players)) +
   facet_wrap(~Players_f + paste("rate: ", round(event_slope_text)), ncol=7)+
  geom_line() +
  theme_bw()
