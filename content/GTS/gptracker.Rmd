---
title: 'Zig-Zag GTS: Garage Power'
author: "giddybb"
date: "`r format(Sys.time(), '%d %B, %Y')`"
output: html_document
summary: Garage power trends for players in Zig-Zag second team.
tags:
- Player
- Detailed
---
...
## The Middle Order (GP 5000-6000)

```{r, echo=FALSE}
gplong%>%
  filter(max_gp<6000, max_gp>5000, n_measures>1, current) %>%
...
## The Tail Enders (GP under 5000)

```{r, echo=FALSE, fig.height=7}
gplong%>%
  filter(max_gp<5000, n_measures>1, current) %>%
  ggplot(aes(event, GP, group=Players)) +
   facet_wrap(~Players_f + paste("rate: ", round(event_slope_text)), ncol=7)+
  geom_line() +
  theme_bw()
